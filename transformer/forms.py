...
class TransformerForm(forms.Form):
	...
	s2_current = forms.FloatField(label='S2 current', required=False, disabled=True)
	s3_enabled = forms.BooleanField(label='S3 enabled', initial=False, required=False)
	s3_voltage = forms.FloatField(label='S3 voltage', required=False, disabled=True)
	s3_current = forms.FloatField(label='S3 current', required=False, disabled=True)
	s4_enabled = forms.BooleanField(label='S4 enabled', initial=False, required=False)
	s4_voltage = forms.FloatField(label='S4 voltage', required=False, disabled=True)
	s4_current = forms.FloatField(label='S4 current', required=False, disabled=True)
